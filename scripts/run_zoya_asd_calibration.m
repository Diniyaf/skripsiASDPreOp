...
% RUN_ZOYA_ASD_CALIBRATION
% -------------------------------------------------------------------------
% Staged calibration for Patient Zoya ASD pre-closure.
% Methodology: mirrors Hafiz-Keisya unified VSD run_calibration.m.
%
...
run_dir = fullfile(output_dir, sprintf('zoya_asd_calib_%s', timestamp));
...
ctx = run_asd_patient_case(@patient_zoya, 'zoya', 'pre_surgery', ctx_options);
...
gsa_files = dir(fullfile(gsa_dir, 'zoya_asd_gsa_curated_*.mat'));
if isempty(gsa_files)
    gsa_files = dir(fullfile(gsa_dir, 'zoya_asd_gsa_full_*.mat'));
end
if isempty(gsa_files)
    error('No GSA results found. Run scripts/run_zoya_asd_gsa_curated.m first.');
end
...
mat_file = fullfile(run_dir, sprintf('zoya_asd_calibration_%s.mat', timestamp));
save(mat_file, 'pkg');
fprintf('  MAT saved: %s\n', mat_file);
...
csv_base = fullfile(run_dir, sprintf('zoya_asd_baseline_%s.csv', timestamp));
csv_cal  = fullfile(run_dir, sprintf('zoya_asd_calibrated_%s.csv', timestamp));
...
function loc = require_parameter_locations(all_names, required_names, stage_label)
% REQUIRE_PARAMETER_LOCATIONS - locate pre-registered active-set parameters.
[present, loc] = ismember(required_names, all_names);
if ~all(present)
    missing = required_names(~present);
    error('run_zoya_asd_calibration:missingActiveSetParameter', ...
        '%s active-set parameter(s) missing from registry: %s', ...
        stage_label, strjoin(missing, ', '));
end
loc = loc(:);
end
