%% run_indira_asd_gsa_curated.m
% RUN_INDIRA_ASD_GSA_CURATED
% -------------------------------------------------------------------------
% Patient Indira ASD pre-closure curated-parameter Sobol GSA.
% 15yr female, large secundum ASD (22.3mm), ΔP=2mmHg, linear shunt mode.
%
...
excel_path = make_unique_output_path(output_dir, ...
    sprintf('indira_asd_gsa_curated_%s.xlsx', timestamp));
...
fprintf('===============================================================\n');
fprintf('  Patient Indira ASD Curated-Parameter Pre-Calibration GSA\n');
fprintf('===============================================================\n');
...
paths = struct();
paths.ST_heatmap_png = fullfile(figure_dir, sprintf('indira_asd_gsa_curated_ST_heatmap_%s.png', timestamp));
paths.ST_heatmap_pdf = replace_ext(paths.ST_heatmap_png, '.pdf');
paths.S1_heatmap_png = fullfile(figure_dir, sprintf('indira_asd_gsa_curated_S1_heatmap_%s.png', timestamp));
...
for idx = 1:numel(bar_metrics)
    ...
    png_path = fullfile(figure_dir, sprintf('indira_asd_gsa_curated_ST_bar_%s_%s.png', ...
