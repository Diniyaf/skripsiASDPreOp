%% run_indira_asd_calibration.m
% RUN_INDIRA_ASD_CALIBRATION
% -------------------------------------------------------------------------
% Staged calibration for Patient Indira ASD pre-closure.
% 15yr female, large secundum ASD (22.3mm), linear shunt mode (R.asd).
% Has LA-RA gradient → R_ASD seeded from ΔP/Q = 2/4.32 = 0.46 mmHg·s/mL.
% Has RAP+LAP → atrial pressure constrained, may not need atrial expansion.
%
...
run_dir = fullfile(output_dir, sprintf('indira_asd_calib_%s', timestamp));
...
%% ========================================================================
fprintf('=== Loading Patient Indira ASD baseline + GSA ===\n');
fprintf('  MODE: linear shunt (R.asd seeded from reported LA-RA gradient and Qp-Qs)\n');
fprintf('  Target policy: RAP is primary when measured; DeltaP/Q_ASD are mechanism guards.\n\n');

ctx_options = struct('scaling_mode', 'lundquist_bsa', ...
    'runBaselineSimulation', false);
ctx = run_asd_patient_case(@patient_indira, 'indira', 'pre_surgery', ctx_options);
...
gsa_files = dir(fullfile(gsa_dir, 'indira_asd_gsa_curated_*.mat'));
if isempty(gsa_files)
    gsa_files = dir(fullfile(gsa_dir, 'indira_asd_gsa_*_*.mat'));
end
if isempty(gsa_files)
    error('No GSA results found. Run scripts/run_indira_asd_gsa_curated.m first.');
end
...
% Indira has measured RAP and LAP, so atrial/preload parameters are not
% force-added from the Zoya v4 exploration. They enter only if supported by
% the GSA active-mask rule below.
...
if use_par
    pool = gcp('nocreate');
    if isempty(pool)
        warning('run_indira_asd_calibration:noPoolForFmincon', ...
