%% run_aluna_asd_gsa_curated.m
% RUN_ALUNA_ASD_GSA_CURATED
% -------------------------------------------------------------------------
% Patient Aluna ASD pre-closure curated-parameter Sobol GSA.
% 1yr 5mo female, large secundum ASD (17.55 mm), no measured LA-RA
% pressure gradient, no Qp/Qs, and no direct shunt-flow data. The seeded
% ASD mechanism is therefore geometry-based/orifice mode when the clinical
% mapper has ASD diameter but cannot compute a linear R.asd from DeltaP/Q.
%
...
%   -> pediatric scaling using Aluna BSA/anthropometry
...
excel_path = make_unique_output_path(output_dir, ...
    sprintf('aluna_asd_gsa_curated_%s.xlsx', timestamp));
mat_path = make_unique_output_path(output_dir, ...
    sprintf('aluna_asd_gsa_curated_%s.mat', timestamp));
...
fprintf('===============================================================\n');
fprintf('  Patient Aluna ASD Curated-Parameter Pre-Calibration GSA\n');
fprintf('===============================================================\n');
...
ctx = run_asd_patient_case(@patient_aluna, patient_label, scenario, ctx_options);
...
if use_parallel && license('test', 'Distrib_Computing_Toolbox') ~= 1
    warning('run_aluna_asd_gsa_curated:noParallelToolbox', ...
...
if use_parallel
    try
        ...
        warning('run_aluna_asd_gsa_curated:parallelFallback', ...
...
for idx = 1:numel(names)
    ...
    switch numel(parts)
        ...
        otherwise
            error('run_aluna_asd_gsa_curated:unsupportedParameterDepth', ...
...
[is_present, loc] = ismember(selected_metrics, all_metrics);
if ~all(is_present)
    error('run_aluna_asd_gsa_curated:missingMatrixMetric', ...
...
paths = struct();
paths.ST_heatmap_png = fullfile(figure_dir, sprintf('aluna_asd_gsa_curated_ST_heatmap_%s.png', timestamp));
paths.ST_heatmap_pdf = replace_ext(paths.ST_heatmap_png, '.pdf');
paths.S1_heatmap_png = fullfile(figure_dir, sprintf('aluna_asd_gsa_curated_S1_heatmap_%s.png', timestamp));
...
for idx = 1:numel(bar_metrics)
    ...
    png_path = fullfile(figure_dir, sprintf('aluna_asd_gsa_curated_ST_bar_%s_%s.png', ...
